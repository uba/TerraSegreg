...
ExposureIndexMNWin::ExposureIndexMNWin(PluginParameters* pp)
: SpatialIndexWin(pp)
{
	...
	m_spatialIndex = new ExposureIndexMN();
	m_helpPath = "TerraSegreg/ExposureIndexMN.html";
} 

ExposureIndexMNWin::~ExposureIndexMNWin()
{}

void ExposureIndexMNWin::verifyPushButton_clicked()
{
    ...
    for(it = m_groups.begin(); it != m_groups.end(); ++it)
    {
		...
		groupNComboBox->insertItem(it->first.c_str());
        row++;
    }
}

bool ExposureIndexMNWin::configureSpatialIndex()
{
	// Configures Spatial Exposure Index of Group m to Group n.
	ExposureIndexMN* ei = dynamic_cast<ExposureIndexMN*>(m_spatialIndex);

	std::string gm = groupMComboBox->currentText().latin1();
	std::string gn = groupNComboBox->currentText().latin1();
...
void ExposureIndexMNWin::update(TeSTElementSet* objSet)
{
	...
	QMessageBox::information(this, tr("Information"), tr("Spatial Exposure Index of Group m to Group n calculated with successful."));
}
