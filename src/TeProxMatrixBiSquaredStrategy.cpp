...
/*!
	\file TeProxMatrixBiSquaredStrategy.cpp
...
TeProxMatrixBiSquaredStrategy::TeProxMatrixBiSquaredStrategy(const double& b) 
	: TeProxMatrixWeightsStrategy()
{
	params_.max_local_dist_ = b;
}
    
TeProxMatrixBiSquaredStrategy::~TeProxMatrixBiSquaredStrategy()
{}

bool TeProxMatrixBiSquaredStrategy::ComputeWeigths(TeProxMatrixImplementation* imp)
{
	if(imp == 0) 
		return false;
	...
	for(int i = 0; i < n; ++i) // for each object
	{
		...
		if(imp->getNeighbours(i, objectid, neighbours))
		{
			...
			for(int j = 0; j < size; ++j) // for each neighbour
			{
				...
				// Calculates the weight from bi-squared strategy
				double w = pow((1 - pow((d / b), 2)), 2);
